<?php
     ...
     */
    /*
    Plugin Name: POWr Popup
    Plugin URI: https://www.powr.io/plugins/popup
    Description: Capture contacts and promote offers with custom popups  Drop the widget anywhere in your theme. Or use the POWr icon in your WP text editor to add to a page or post. Edit on your live page by clicking the settings icon. More plugins and tutorials at POWr.io.
    Author: POWr.io
    Version: 1.6.4
    ...
    */
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////
    ///////////////////////////////////////Create Popup widget/////////////////////////////////
    class Powr_Popup extends WP_Widget{
      public $plugin;
      //Create the widget
      public function __construct(){
        parent::__construct( 'powr_popup',
                             __( 'POWr Popup' ),
                             array( 'description' => __( 'Popup by POWr.io') )
        );
      ...
      public function widget( $args, $instance ){
        ...
        ?>
        <div class='widget powr-popup' label='<?php echo $label; ?>'></div>
        <?php
      }
      ...
      public function form( $instance ){
        ...
        ?>
        <p>
          To edit, visit your live webpage and click the gears icon on your Popup.
        </p>
        <p>
    ...
    function register_powr_popup() {
      register_widget( 'Powr_Popup' );
    }
    //Use widgets_init action hook to execute custom function
    add_action( 'widgets_init', 'register_powr_popup' );
    //Create short codes for adding plugins anywhere:
    function powr_popup_shortcode( $atts ){
      if(isset($atts['id'])){
        $id = $atts['id'];
      	return "<div class='powr-popup' id='$id'></div>";
      }else if(isset($atts['label'])){
        $label = $atts['label'];
		    return "<div class='powr-popup' label='$label'></div>";
      }else{
      	"<div class='powr-popup'></div>";
      }
    }
    add_shortcode( 'powr-popup', 'powr_popup_shortcode' );
    ...
    //ADD MENUS
    add_action( 'admin_menu', 'powr_popup_menu' );
    function powr_popup_menu() {
      add_menu_page( 'POWr Popup', 'POWr Popup', 'manage_options', 'powr-popup-settings', 'powr_popup_options', plugins_url('/powr-icon.png',__FILE__));
    }
    function powr_popup_options() {
      if(powr_local_mode()){//Determine JS url:
        $redirect_url = 'https://localhost:3000/wp-create/popup';
      }else{
        $redirect_url = 'https://www.powr.io/wp-create/popup';
      }
    ...
    register_activation_hook(__FILE__, 'powr_popup_plugin_activate');
      add_action('admin_init', 'powr_popup_plugin_redirect');

      function powr_popup_plugin_activate() {
      add_option('powr_popup_plugin_do_activation_redirect', true);
      }
      ...
      function powr_popup_plugin_redirect() {
      if (get_option('powr_popup_plugin_do_activation_redirect', false)) {
          delete_option('powr_popup_plugin_do_activation_redirect');
          if(!isset($_GET['activate-multi']))
          {
            wp_redirect( get_admin_url().'?platform=wordpress&page=powr-popup-settings&' );
          }
       }
      }
